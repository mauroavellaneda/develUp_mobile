...
describe("Visitor can see public index", () => {
  ...
  it("is expected to have elements within assignment card", async () => {
    ...
    await expect(element(by.id("title-45"))).toHaveLabel("Build a Web page");
    await expect(element(by.id("budget-45"))).toHaveLabel("$ 500");
    await expect(element(by.id("points-45"))).toHaveLabel("Points: 320");
  });
  it("is expected to have ability to scroll up", async () => {
    ...
    await expect(element(by.id("title-14"))).toHaveLabel(
      "Build a Full Stack Rails application"
    );
